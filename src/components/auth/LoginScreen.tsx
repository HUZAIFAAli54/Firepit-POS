...
import { db, hashPin, getSetting } from '../../db/database';
...
export default function LoginScreen() {
  ...
  const handleResetPINs = async () => {
    ...
    setResetError('');
    try {
      // The reset is gated behind the recovery code (shown in Admin → Settings).
      // Without it, anyone at the terminal could seize the admin account.
      const stored = await getSetting('recoveryCode');
      if (!stored || recoveryCodeInput.trim().toUpperCase() !== stored.toUpperCase()) {
        setResetError('Incorrect recovery code. Ask the shop owner for the code in Admin → Settings.');
        return;
      }
      ...
      await logActivity(0, 'System', 'system', 'auth', 'PINs Reset via Recovery Code', 'All admin/cashier PINs were reset to factory defaults');
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-amber-50 via-orange-50 to-amber-100 flex flex-col items-center justify-center p-4">
      ...
      <button
        onClick={() => { setShowForgotPin(true); setResetDone(false); setRecoveryCodeInput(''); setResetError(''); }}
            ...
            {resetDone ? (
              ...
              <>
                ...
                <input
                  ...
                  onChange={e => { setRecoveryCodeInput(e.target.value.toUpperCase().replace(/[^A-Z0-9]/g, '')); setResetError(''); }}
                  placeholder="8-character code"
                  maxLength={8}
                  className="w-full px-4 py-2.5 border border-gray-200 rounded-xl font-mono tracking-widest text-center uppercase focus:outline-none focus:ring-2 focus:ring-amber-300 mb-2"
                />
                <p className="text-xs text-gray-400 mb-4">
                  The shop owner can find this code in Admin → Settings → Security.
                </p>
                {resetError && (
                  <p className="text-xs text-red-600 bg-red-50 border border-red-200 rounded-lg px-3 py-2 mb-4">{resetError}</p>
                )}
